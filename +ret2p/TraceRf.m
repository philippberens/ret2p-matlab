%{
ret2p.TraceRf (imported) # receptive field using dense noise
...
classdef TraceRf < dj.Relvar & dj.AutoPopulate
    properties(Constant)
        table = dj.Table('ret2p.TraceRf');
        popRel = (ret2p.TraceRfParams * ret2p.Trace) & ret2p.Stimulus('stim_type="DN"');
    end
    
    methods
        function self = TraceRf(varargin)
