%{
ret2p.ReceptiveFieldPatchedCell (imported) # receptive field using dense noise
...
classdef ReceptiveFieldPatchedCell < dj.Relvar & dj.AutoPopulate
    properties(Constant)
        table = dj.Table('ret2p.ReceptiveFieldPatchedCell');
        popRel = ret2p.Spikes & ret2p.Stimuli('stim_type="DN"');
    end
    ...
    methods(Access = protected)
        function makeTuples(self, key)
            ...
            %             spikes = fetch1(ret2p.Spikes(key) & dnStim,'spikes');
            %             spikes = spikes{:};
            [trace t_time] = fetch1(ret2p.Spikes(key) & dnStim,'ca_trace','time');
            ...
            %             spikes(spikes<=s_time_hr(a_idx))=[];
            %             spikes(spikes>s_time_hr(end-b_idx))=[];
            %
            %             tstim = zeros(size(stim,1),S,length(spikes));
            %             for i=1:length(spikes)
            %                 [~,idx]=min(abs(s_time_hr-spikes(i)));
            %                 tstim(:,:,i) = stim(:,(idx-a_idx+1):idx+b_idx);
            %             end
            mtstim = mean(tstim,3);
